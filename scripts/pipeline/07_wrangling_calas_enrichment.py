...
def match_nearest(df, ds, var):
    """Fast nearest-neighbor match: load array once, then index with pure numpy."""
    da   = ds[var].squeeze()                          # drop any size-1 dims (e.g. depth)
    data = da.values                                  # (T, lat, lon) in RAM
    ...
    # Vectorized nearest index via broadcasting (O(M*N), M<=365, N<=15k -- fast)
