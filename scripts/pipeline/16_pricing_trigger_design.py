...
Outputs:
  PLOTS/16_pricing_spatial_comparison.png
...
# ── fishing polygon (p5-p95 cala longitudes per 1-deg lat band) ───────────────
def build_fishing_polygon_coords(lat_min=-15.8, lat_max=None):
    """Returns (west_lons, east_lons, band_lats) arrays for the fishing corridor."""
    df = pd.read_csv(
...
BETA_TRIGGER  = -0.519   # Centro Norte conservador (IC95% upper bound, betas_by_region.csv)

STEP_TRIGGER  = 1.116    # deg C: p90 of historical seasonal SST distribution

# Historical quantiles (Centro Norte T1+T2 pooled, climatology 2005-2024, triggers_centro_norte.json)
HIST_QUANTILES = {
    "p90": 1.116,
    "p95": 1.752,
    "p99": 2.715,
...
ENTRY_SST = HIST_QUANTILES["p90"]   # ramp entry = p90
EXIT_SST  = HIST_QUANTILES["p99"]   # ramp exit  = p99
...
# ── main ─────────────────────────────────────────────────────────────────────
def main():
    out_spatial = PLOTS / "16_pricing_spatial_comparison.png"
    out_payout  = PLOTS / "16_pricing_payout_curves.png"
