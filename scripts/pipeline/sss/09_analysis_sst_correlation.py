"""
Step 17b - Correlacion SSS vs SST por region

Mismo formato que 16b pero Y = anomalia SST promedio x temporada,
X = anomalia SSS promedio x temporada, a nivel empresa x temporada.

Inputs:
  OUTPUTS/calas_enriched.csv   (columnas oisss_sss_anom y modis_sst_anom)

Outputs:
  PLOTS/17b_analysis_sss_sst_correlation.png
...
def build_fishing_polygon_coords(lat_min=-15.8, lat_max=None):
    ...
    return poly_lons, poly_lats, band_lats
...
def plot_zone_map(ax):
    ...
    for label, lat_s, lat_n, color in ZONE_BANDS:
        ...
        ax.text(-81.5, (lat_s + lat_n) / 2, label,
                transform=ccrs.PlateCarree(),
                fontsize=7, va="center", color=color, fontweight="bold", zorder=4)
    for lat in [-7.1, -11.0, -15.8]:
        ax.plot([lon_w, lon_e], [lat, lat], transform=ccrs.PlateCarree(),
                color="black", lw=0.8, ls="--", alpha=0.5, zorder=4)
        ax.text(-73.8, lat, f"{abs(lat):.1f}°S", transform=ccrs.PlateCarree(),
                fontsize=6.5, va="center", color="#333333", zorder=5)
    result = build_fishing_polygon_coords()
    if result[0] is not None:
        poly_lons, poly_lats, _ = result
...
def run_ols(sub):
    ...
    ).reset_index().dropna(subset=["mean_sss", "mean_sst"])
    if len(agg) < 10:
        return None, agg
    m = smf.ols("mean_sst ~ mean_sss", data=agg).fit()
    return m, agg


def main():
    out = PLOTS / "17b_analysis_sss_sst_correlation.png"
    if out.exists():
        print("17b output exists -- skipping")
        return

    df = pd.read_csv(OUTPUTS / "calas_enriched.csv", low_memory=False)
    df["date"] = pd.to_datetime(df["date"])
    df = df.dropna(subset=["oisss_sss_anom", "modis_sst_anom", "catch_tm",
                            "lat", "season", "company"])
    df = df[df["catch_tm"] > 0]
    ...
    print("\nCorrelacion SSS ~ SST (empresa x temporada):")
    for col, (name, lat_min, lat_max) in enumerate(SCOPES):
        ...
        ax_sc.scatter(agg["mean_sss"], agg["mean_sst"],
                      color=col_c, alpha=0.6, s=20, zorder=3)

        if m is not None:
            x_line = np.linspace(agg["mean_sss"].min() - 0.02,
                                 agg["mean_sss"].max() + 0.02, 100)
            ax_sc.plot(x_line,
